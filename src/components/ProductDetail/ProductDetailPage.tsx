...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        // Try to log product view once user is known and product available
        if (product && !hasLoggedViewRef.current) {
          logProductView(user.uid, product);
        }
      }
    });
    return () => unsub();
  ...
  // Log product view when product is loaded and user is logged in
  useEffect(() => {
    const user = auth.currentUser;
    if (user && product && !hasLoggedViewRef.current) {
      logProductView(user.uid, product);
    }
  }, [product]);
  ...
  const logProductView = async (uid: string, p: Product) => {
    try {
      const activityId = `product_view_${p.ASIN}_${Date.now()}`;
      await setDoc(doc(db, 'users', uid, 'activities', activityId), {
        type: 'product_view',
        title: `Lihat ${p.nama_produk}`,
        productASIN: p.ASIN,
        store: p.toko || null,
        category: p.kategori || null,
        image: p.gambar_produk || p.thumbnail_produk || (p.product_photos && p.product_photos[0]) || null,
        createdAt: serverTimestamp()
      });
      hasLoggedViewRef.current = true;
    } catch (e) {
      // Silent fail; logging is non-blocking
      console.warn('Gagal mencatat view produk:', e);
    }
  };
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4">
              {relatedProducts.map((item) => (
                <div
                  key={item.ASIN}
                  onClick={() => { try { localStorage.setItem('selectedProduct', JSON.stringify(item)); } catch {}; window.location.href = '/BuyingPage'; }}
